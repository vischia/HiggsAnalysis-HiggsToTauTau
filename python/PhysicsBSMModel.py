...
class MSSMLikeHiggsModel(PhysicsModel):
    ...
    def getYieldScale(self,bin,process):
        ...
        if processSource not in ["ggH", "bbH", "HTB", "TBH"]:
        ...
        for D in [ "hbb", "htt", "hmm" ]:
            ...
            if D in decaySource:
                ...
                foundDecay = D
        if not foundDecay: print "ciao" ## RuntimeError, "Validation Error: decay string %s does not contain any known decay name" % decaySource
        foundEnergy = None
        for D in [ "7TeV", "8TeV" ]:
            if D in decaySource:
                if foundEnergy: print "ciao" ##raise RuntimeError, "Validation Error: decay string %s contains multiple known energies" % decaySource
                foundEnergy = D
        if not foundEnergy:
...
class FloatingMSSMXSChargedHiggs(MSSMLikeHiggsModel):
    """
    Trivial model to float ggH and bbH independently. At the moment only ggH and bbh are supported. Extensions to the other
    production channels channels are given in comments. Also the principle how to deal with manipulations of the POI's and
    other defined roofit variables are shown in comments. 
        ...
        for po in physOptions:
            ...
            if po.startswith("mARange="):
                ...
                if len(self.mARange) != 2:
                    ...
                    raise RuntimeError, "Extrema for mA range defined with inverterd order. Second element must be larger than first element"
            if po.startswith("HTBRange="):
                self.HTBRange = po.replace("HTBRange=","").split(":")
                if len(self.HTBRange) != 2:
                    raise RuntimeError, "HTB signal strength range requires minimal and maximal value"
                elif float(self.HTBRange[0]) >= float(self.HTBRange[1]):
                    raise RuntimeError, "minimal and maximal range swapped. Second value must be larger first one"
            if po.startswith("TBHRange="):
                self.TBHRange = po.replace("TBHRange=","").split(":")
                if len(self.TBHRange) != 2:
                    raise RuntimeError, "TBH signal strength range requires minimal and maximal value"
                elif float(self.TBHRange[0]) >= float(self.TBHRange[1]):
                    raise RuntimeError, "minimal and maximal range swapped. Second value must be larger first one"                 
    def doParametersOfInterest(self):
        """
        Create POI and other parameters, and define the POI set. E.g. Evaluate cross section for given values of mA and tanb
...
## this is the entry point for text2workspace.py
floatingChargedHiggs = FloatingMSSMXSChargedHiggs()
